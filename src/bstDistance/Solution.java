...
/*
 * This program finds the distance between two nodes in a Binary Tree
 * If X,Y are two nodes, Distance(X,Y) = Distance(root,X) + Distance(root,Y) - 2*Distance(root,LCA(X,Y))
 * LCA = Least Common Ancestor, Distance(root,X) = number of edges between root and X(number of vertices including root - 1)
 * We subtract the distance between root and LCA because its gets added in the individual distances calculated
 */
class Node{
	int data;
	Node left;
